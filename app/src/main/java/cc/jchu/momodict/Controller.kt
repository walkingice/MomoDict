...
class Controller(
    ...
    private val recordTimeComparator: Comparator<Record> =
        Comparator { left, right ->
            if (left.time!!.before(right.time)) 1 else -1
        }

    // sorting by time. Move latest one to head
    private val cardTimeComparator =
        Comparator<Card> { left, right ->
            if (left.time!!.before(right.time)) 1 else -1
        }
    ...
    suspend fun getEntries(keyWord: String): List<Entry> =
        withContext(Dispatchers.IO) {
            // to make sure exact matched words are returned
    ...
    suspend fun setRecord(record: Record): Boolean =
        withContext(Dispatchers.IO) {
            mStore.upsertRecord(record)
        }

    suspend fun removeRecord(keyWord: String): Boolean =
        withContext(Dispatchers.IO) {
    ...
    suspend fun setCard(card: Card): Boolean =
        withContext(Dispatchers.IO) {
            mStore.upsertCard(card)
        }

    suspend fun removeCards(keyWord: String): Boolean =
        withContext(Dispatchers.IO) {
